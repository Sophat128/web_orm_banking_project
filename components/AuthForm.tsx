...
const AuthForm = ({ type }: { type: string }) => {
  ...
  // Fetch city and state based on postal code
  const fetchCityAndState = async (postalCode: string) => {
    try {
  ...
  const handlePostalCodeChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (postalCode.length === 5) {
      await fetchCityAndState(postalCode);
    }
  };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              {type === 'sign-up' && (
                <>
                  ...
                  <div className="flex gap-4">
                    <CustomInput control={form.control} name='state' label="State" placeholder='Example: NY' disabled/>
                  <CustomInput control={form.control} name='city' label="City" placeholder='Enter your city' disabled />
